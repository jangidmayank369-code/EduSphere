...
def create_school(
    ...
    school = SchoolService(db).create(
        **payload.model_dump(),
    )
...
# ---------------------------------------------------------------------------
# Academic Session static routes
# These MUST come before /{school_id}
# ---------------------------------------------------------------------------
...
def get_academic_session(
    ...
    session = AcademicSessionService(db).get(
        session_id,
    )
...
def set_current_academic_session(
    ...
    session = AcademicSessionService(db).set_current(
        session_id,
    )
...
def update_school(
    ...
    db: Session = Depends(get_db),
    _: User = Depends(
        require_permission("SCHOOL_UPDATE")
    ),
):
...
def list_academic_sessions(
    ...
    return {
        "success": True,
        "data": [
            AcademicSessionResponse.model_validate(
                session
            ).model_dump()
...
def get_current_academic_session(
    ...
    session = AcademicSessionService(db).get_current(
        school_id,
    )
